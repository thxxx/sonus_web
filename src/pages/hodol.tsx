...
export default function WebRealtimeTranscriber() {
  ...
  const openSocket = useCallback(async () => {
    const endPoint = "ws";
    const url = `wss://lcoapo6t37uz5g-5000.proxy.runpod.net/${endPoint}`;
    ...
    ws.onmessage = async (ev) => {
      ...
      if (ev.data instanceof ArrayBuffer) {
        ...
        if (view.getUint8(0) === 0xa1 && view.getUint8(1) === 0x51) {
          ...
          if (!payload || payload.length === 0) {
            console.warn("[OpusDecoder] skip empty payload isFinal ", isFinal);
      ...
      if (msgType === "delta") {
        ...
        // ✅ 구(PCM/MP3) 호환용: 서버가 여전히 tts_audio(JSON)로 줄 때만 사용
        ...
        setMyTranscripts((ts) => [msg.text, ...ts.slice(1, -1)]);
        setAnswers((ts) => [msg.text, ...ts.slice(1, -1)]);
      }
    };
